...
const LudoHome = ({ onCreateTeam, onJoinTeam, onBackToSelection }) => {
    ...
    if (mode === "join") {
      const requestTeams = () => {
        ...
        if (socket && socket.connected) {
          console.log("Requesting available Ludo teams from server...");
          socket.emit("ludo-get-teams");

          const handleTeamsList = (teams) => {
            console.log("Available Ludo teams received:", teams);
          };

          socket.on("ludo-teams-list", handleTeamsList);

          return () => {
            socket.off("ludo-teams-list", handleTeamsList);
          };
        } else {
